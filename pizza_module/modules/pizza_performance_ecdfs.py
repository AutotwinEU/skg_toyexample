...
from pizza_module.modules.plot import *

class PizzaPerformanceModuleEcdfs:
    def __init__(self, db_connection):
        self.connection = db_connection

    def ecdfc_latency_between_sensors(self, isensor, osensor, gt_sim):
        exec_times = []
        if gt_sim == "sim":
            exec_times = self.connection.exec_query(pfql.execution_times_between_sensors_sim,
                                                ** {"isensor": isensor, "osensor": osensor })
        if gt_sim == "gt":
            exec_times = self.connection.exec_query(pfql.execution_times_between_sensors_gt,
                                                ** {"isensor": isensor, "osensor": osensor })
    ...
    def create_ecdf_collections_for_latencies(self,gt_sim):
        ecdfcs = []
        ...
        for sensors in sensor_connections: # + sensor_connections_full_path:
            for osensor in sensors["outputs"]:
                ecdfcs.append(self.ecdfc_latency_between_sensors(sensors["input"], osensor, gt_sim))
        return ecdfcs
    ...
    def return_ecdfcs(self,gt_sim):
        return self.create_ecdf_collections_for_latencies(gt_sim) #+ self.create_ecdf_collections_for_queues()
