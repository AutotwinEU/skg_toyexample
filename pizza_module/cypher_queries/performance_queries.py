...
class PerformanceQueryLibrary:
    ...
    @staticmethod
    def execution_times_between_sensors_sim(isensor, osensor):
    ...
    @staticmethod
    def execution_times_between_sensors_gt(isensor, osensor):
        query_str = '''MATCH path=(ev:Event)- [:DF_CONTROL_FLOW_ITEM*]->(g:Event)
                WHERE NOT EXISTS((:Event)-[:DF_CONTROL_FLOW_ITEM]->(ev))
                AND NOT EXISTS((g)-[:DF_CONTROL_FLOW_ITEM]->(:Event))
                AND ev.simulated is null
                UNWIND nodes(path) as e1
                UNWIND nodes(path) as e2
                WITH e1,e2
                match (e1)-[:EXECUTED_BY]->(s1)
    ...
    @staticmethod
    def store_in_db(kind, name, value, gt_sim):
        query_str = '''create(n1:$kind:Performance{name:"$name",val:"$value",source:"$gt_sim"})
                RETURN id(n1) as id'''
        return Query(query_str=query_str,
                     template_string_parameters={"kind": kind, "name":name, "value": value, "gt_sim":gt_sim }
                     )

    # retrieves a performance artifact based on a name
    @staticmethod
    def retrieve_from_db(name,gt_sim):
        query_str = '''match(n:Performance{name:"$name"}) 
                        where n.source="$gt_sim"
                        return n.val as value'''
        return Query(query_str=query_str,
                     template_string_parameters={"name": name, "gt_sim":gt_sim}
                     )

    @staticmethod
    def retrieve_from_db_all(name):
        query_str = '''match(n:Performance{name:"$name"})
                         return n.val as value'''
        return Query(query_str=query_str,
    ...
    def retrieve_performance_artifacts_from_db(kind):
        query_str = '''match(n: $kind:Performance) return distinct(n.name) as names'''
        return Query(query_str=query_str,
    ...
    @staticmethod
    def all_ecdfs_of_an_ecdfc(name,gt_sim):
        query_str = '''MATCH(n:Performance:Latency{name:"$name"})-[:HAS_PERFORMANCE]->(ecdf) 
                        WHERE n.source="$gt_sim"
                        RETURN ecdf.name as ecdf'''
        return Query(query_str=query_str,
                     template_string_parameters={"name": name, "gt_sim": gt_sim}
                     )
    def all_ecdfs_of_an_ecdfc_all(name):
        query_str = '''MATCH(n:Performance:Latency{name:"$name"})-[:HAS_PERFORMANCE]->(ecdf) 
                        RETURN ecdf.name as ecdf'''
        return Query(query_str=query_str,
                     template_string_parameters={"name": name}
    ...
    @staticmethod
    def set_queue_properties(name,min,max,average,gt_sim):
        query_str = '''MATCH(p: Queue{name: "$name"})
                       WHERE p.source="$gt_sim"
                       SET p.max = $max
                       SET p.min = $min
                       SET p.average = $average
                       RETURN p'''
        return Query(query_str=query_str,
                     template_string_parameters={"name": name, "min": min, "max": max, "average": average, "gt_sim": gt_sim} )

    @staticmethod
    def set_ecdf_properties(name,min,max,average,median,gt_sim):
        query_str = '''MATCH(p: LatencyECDF{name: "$name"})
                       ...
                       RETURN p'''
        return Query(query_str=query_str,
            template_string_parameters={"name": name, "min": min, "max": max, "average": average, "median": median, "gt_sim": gt_sim} )
    ...
    @ staticmethod
    def connect_latencies_to_ecdf_latencies():
        query_str = ''' match (la:Latency)
                        detach delete la
                        with la
                        match (l:LatencyECDF) 
                        with distinct(l.name) as dl
                        create (p:Performance:Latency{name:dl})
                        with p,dl
                        match (le:LatencyECDF{name:dl})
                        with  p,le
                        create (p)-[:HAS_PERFORMANCE]->(le)'''
        return Query(query_str=query_str)

    @ staticmethod
    def add_suffix_to_ecdf_latencies():
        query_str = '''match(lec: LatencyECDF) SET lec.name = lec.name + " " + lec.source'''
        return Query(query_str=query_str)