...
class Ecdf_compute:
    ...
    # TODO: to test
    @staticmethod
    def kolmogorov(ecdf1: Ecdf, ecdf2: Ecdf):
        return stats.ks_2samp(ecdf1.values(), ecdf2.values()).pvalue
    ...
    def difference(ecdf1: Ecdf, ecdf2: Ecdf):
        previous_value = 0
        area = 0
        for value in sorted(list(set(ecdf1.values() + ecdf2.values()))):
    ...
    # efficient version of previous version that uses the already computed difference
    @staticmethod
    def similarity_using_diff(diff, ecdf1, ecdf2):
        if ecdf1.num_values() == 0 or ecdf2.num_values() == 0:
            return 1
        maximum = max(ecdf1.max_value(), ecdf2.max_value())
        if maximum == 0:
...
def experiment1():
    test = Ecdf(
        [1, 4, 7, 10, 100, 105, 108, 111, 112, 114, 115, 116, 117, 130, 135, 136, 138, 140, 160, 180, 200, 205, 210,
         300, 350, 400, 600, 800, 1000, 2000, 5000, 8000, 10000, 20000, 40000], "Original")
    test.print()
    ...
    test4 = Ecdf([1,10,100,200,250,260,280,300,500,1000])
    test4.print()
    print(Ecdf_compute.similarity(test, test4))
