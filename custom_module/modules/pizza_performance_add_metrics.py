...
def add_aggregated_data_to_an_ecdf(dbconnection,ecdf_name):
    ecdf = Store_in_db.retrieve(dbconnection,ecdf_name["names"])
    min = ecdf.min_value()
    max = ecdf.max_value()
...
def add_aggregated_data_to_queues(dbconnection,queue_name):
    queue=Store_in_db.retrieve(dbconnection,queue_name["names"])
    min = queue.get_min()
    max = queue.get_max()
...
def add_metrics(dbconnection):
    print("Starting adding metrics...")

    # add difference, similarity and performance between ecdfs
    for ecdfc_name in Store_in_db.retrieve_names(dbconnection,"Latency"):
    ...
    queue_names=Store_in_db.retrieve_names(dbconnection,"Queue")
